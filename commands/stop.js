...
module.exports.run = async(client, msg, args) => {
    ...
    if (!msg.member.voiceChannel) return msg.channel.send(`${utils.error} No estas en un canal de voz!`);
    if (!serverQueue) return msg.channel.send(`${utils.error} No hay nada para parar de reproducir!`);
    serverQueue.songs = [];
    serverQueue.connection.dispatcher.end(`${utils.info} Se acabo la fiesta.`);
    return undefined;
}
